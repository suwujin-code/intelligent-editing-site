...
function caseRow(study, index) {
  return `<button class="deliverable-row ${index === 0 ? 'active' : ''}" type="button" data-case="${study.id}" aria-label="播放案例：${study.title}">
    <span class="deliverable-preview" aria-hidden="true"><video muted preload="metadata" playsinline><source src="${mediaUrl(study)}#t=0.5" type="video/mp4" /></video><span class="preview-label">播放案例</span></span>
